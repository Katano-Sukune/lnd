...
func calcStaticFee(numHTLCs int) xpcutil.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = xpcutil.Amount(250/4) * 1000
	)
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := xpcutil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += xpcutil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]xpcutil.Address, 0, numOutputs)
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        500,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := xpcutil.NewAmount(5)
	if err != nil {
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	fundingAmount, err := xpcutil.NewAmount(8)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	fundingAmount, err := xpcutil.NewAmount(44)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	req := &lnwallet.InitFundingReserveMsg{
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numBTC = 4
	fundingAmount, err := xpcutil.NewAmount(numBTC)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}

	feePerKw := lnwallet.SatPerKWeight(
		numBTC * numBTC * xpcutil.SatoshiPerBitcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:       chainHash,
		NodeID:          bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// towards Bob's side.
	fundingAmt, err := xpcutil.NewAmount(4)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	pushAmt := lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	if err != nil {
		t.Fatalf("unable to query fee estimator: %v", err)
	}
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = xpcutil.SatoshiPerBitcoin
	...
	for _, txDetail := range txDetails {
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			var destinationAddresses []xpcutil.Address
...
func testTransactionSubscriptions(miner *rpctest.Harness,
	...
	const (
		outputAmt = xpcutil.SatoshiPerBitcoin
		numTxns   = 3
	)
...
func testPublishTransaction(r *rpctest.Harness,
	...
	pubkeyHash := xpcutil.Hash160(pubKey.PubKey.SerializeCompressed())
	keyAddr, err := xpcutil.NewAddressWitnessPubKeyHash(pubkeyHash,
	...
	txFromOutput := func(tx *wire.MsgTx, payToPubKey *btcec.PublicKey,
		txFee xpcutil.Amount) *wire.MsgTx {
		// Create a script to pay to.
		payToPubkeyHash := xpcutil.Hash160(payToPubKey.SerializeCompressed())
		payToKeyAddr, err := xpcutil.NewAddressWitnessPubKeyHash(payToPubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		}
	...
	newTx := func() *wire.MsgTx {
		...
		txFee := xpcutil.Amount(0.1 * xpcutil.SatoshiPerBitcoin)
	...
	// into the mempool.
	txFee := xpcutil.Amount(0.05 * xpcutil.SatoshiPerBitcoin)
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		pubkeyHash := xpcutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := xpcutil.NewAddressWitnessPubKeyHash(pubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		...
		vm, err := txscript.NewEngine(keyScript,
			...
			nil, int64(xpcutil.SatoshiPerBitcoin))
		if err != nil {
			t.Fatalf("unable to create engine: %v", err)
		}
		if err := vm.Execute(); err != nil {
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    xpcutil.SatoshiPerBitcent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate)
	txHash = tx.TxHash()
