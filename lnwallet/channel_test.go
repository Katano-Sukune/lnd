...
func TestSimpleAddSettleWorkflow(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin)
	...
	mSatTransferred := lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin)
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(xpcutil.NewTx(commitTx))
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (xpcutil.Amount(500) + htlcTimeoutFee(
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...xpcutil.Amount) (
	...
	belowDust := xpcutil.Amount(500) + htlcTimeoutFee(feePerKw)
	aboveDust := xpcutil.Amount(1400) + htlcSuccessFee(feePerKw)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := xpcutil.Amount(xpcutil.SatoshiPerBitcoin*4) -
		calcStaticFee(1)
	...
	expectedBalance := xpcutil.Amount(xpcutil.SatoshiPerBitcoin * 5)
	if aliceChannel.channelState.LocalCommitment.LocalBalance.ToSatoshis() !=
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal xpcutil.Amount) {
	...
	// satoshis.
	dustLimit := xpcutil.Amount(10000)
	...
	aliceFee := xpcutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	...
	bobFee := xpcutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	...
	aliceBal := lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin)
...
func TestUpdateFeeConcurrentSig(t *testing.T) {
	...
	htlc := &lnwire.UpdateAddHTLC{
		PaymentHash: paymentHash,
		Amount:      xpcutil.SatoshiPerBitcoin,
		Expiry:      uint32(5),
	}
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * xpcutil.SatoshiPerBitcoin)
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * xpcutil.SatoshiPerBitcoin)
	const numHTLCs = 30
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(4.1 * xpcutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(1 * xpcutil.SatoshiPerBitcoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMSatFromSatoshis(xpcutil.SatoshiPerBitcoin * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshis(1.5 * xpcutil.SatoshiPerBitcoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance xpcutil.Amount) {
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel, func()) {
		...
		aliceMinReserve := xpcutil.Amount(0.5 *
			xpcutil.SatoshiPerBitcoin)
		...
		bobMinReserve := xpcutil.Amount(6 * xpcutil.SatoshiPerBitcoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * xpcutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		...
		xpcutil.SatoshiPerBitcoin*4.5-commitFee, xpcutil.SatoshiPerBitcoin*5,
	)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshis(3.5 * xpcutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		xpcutil.SatoshiPerBitcoin*3-commitFee, xpcutil.SatoshiPerBitcoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		xpcutil.SatoshiPerBitcoin*3-commitFee, xpcutil.SatoshiPerBitcoin*7,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		xpcutil.SatoshiPerBitcoin*3-commitFee, xpcutil.SatoshiPerBitcoin*6,
	)
}
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMSatFromSatoshis(0.1 * xpcutil.SatoshiPerBitcoin)
