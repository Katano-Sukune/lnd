...
type testContext struct {
	...
	fundingChangeAddress xpcutil.Address
	fundingInputUtxo     *Utxo
	fundingInputTxOut    *wire.TxOut
	fundingTx            *xpcutil.Tx
...
func newTestContext() (tc *testContext, err error) {
	...
	tc.fundingChangeAddress, err = xpcutil.DecodeAddress(
...
func (tc *testContext) extractFundingInput() (*Utxo, *wire.TxOut, error) {
	...
	block1Utxo := Utxo{
		...
		Value:       xpcutil.Amount(txout.Value),
...
func TestCommitmentSpendValidation(t *testing.T) {
	...
	const channelBalance = xpcutil.Amount(1 * 10e8)
