...
func (c *commitment) toDiskCommit(ourCommit bool) *channeldb.ChannelCommitment {
	...
	commit := &channeldb.ChannelCommitment{
		...
		CommitFee:       c.fee,
		FeePerKw:        xpcutil.Amount(c.feePerKw),
		CommitTx:        c.txn,
		CommitSig:       c.sig,
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	remoteDustLimit xpcutil.Amount) (*PaymentDescriptor, error) {
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			...
			Amount: lnwire.NewMSatFromSatoshis(
				xpcutil.Amount(wireMsg.FeePerKw),
			),
			EntryType:                FeeUpdate,
			addCommitHeightRemote:    commitHeight,
			removeCommitHeightRemote: commitHeight,
...
func htlcTimeoutFee(feePerKw SatPerKWeight) xpcutil.Amount {
	return feePerKw.FeeForWeight(input.HtlcTimeoutWeight)
}

// htlcSuccessFee returns the fee in satoshis required for an HTLC success
// transaction based on the current fee rate.
func htlcSuccessFee(feePerKw SatPerKWeight) xpcutil.Amount {
...
func htlcIsDust(incoming, ourCommit bool, feePerKw SatPerKWeight,
	htlcAmt, dustLimit xpcutil.Amount) bool {
...
func (lc *LightningChannel) createCommitmentTx(c *commitment,
	...
	var (
		delay                      uint32
		delayBalance, p2wkhBalance xpcutil.Amount
	)
	if c.isOurs {
	...
	var totalOut xpcutil.Amount
	for _, txOut := range commitTx.TxOut {
		totalOut += xpcutil.Amount(txOut.Value)
	}
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		SettledBalance:          xpcutil.Amount(localBalance),
...
func newOutgoingHtlcResolution(signer input.Signer, localChanCfg *channeldb.ChannelConfig,
	...
	feePerKw SatPerKWeight, dustLimit xpcutil.Amount, csvDelay uint32, localCommit bool,
...
func newIncomingHtlcResolution(signer input.Signer, localChanCfg *channeldb.ChannelConfig,
	commitHash chainhash.Hash, htlc *channeldb.HTLC, keyRing *CommitmentKeyRing,
	feePerKw SatPerKWeight, dustLimit xpcutil.Amount, csvDelay uint32,
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee xpcutil.Amount,
	localDeliveryScript []byte,
	remoteDeliveryScript []byte) ([]byte, *chainhash.Hash, xpcutil.Amount, error) {
...
func (lc *LightningChannel) CompleteCooperativeClose(localSig, remoteSig []byte,
	...
	proposedFee xpcutil.Amount) (*wire.MsgTx, xpcutil.Amount, error) {
...
func (lc *LightningChannel) UpdateFee(feePerKw SatPerKWeight) error {
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		Amount:    lnwire.NewMSatFromSatoshis(xpcutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
	}
...
func CreateCommitTx(fundingOutput wire.TxIn,
	...
	amountToSelf, amountToThem, dustLimit xpcutil.Amount) (*wire.MsgTx, error) {
...
func CreateCooperativeCloseTx(fundingTxIn wire.TxIn,
	localDust, remoteDust, ourBalance, theirBalance xpcutil.Amount,
...
// fee rate (fee-per-kw).
func (lc *LightningChannel) CalcFee(feeRate SatPerKWeight) xpcutil.Amount {
...
func (lc *LightningChannel) LocalChanReserve() xpcutil.Amount {
