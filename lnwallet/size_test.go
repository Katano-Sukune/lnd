...
func TestTxWeightEstimator(t *testing.T) {
	...
	p2pkhAddr, err := xpcutil.NewAddressPubKeyHash(
	...
	p2wshAddr, err := xpcutil.NewAddressWitnessScriptHash(
	...
	p2shAddr, err := xpcutil.NewAddressScriptHash([]byte{0}, netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	for i, test := range testCases {
		...
		expectedWeight := blockchain.GetTransactionWeight(xpcutil.NewTx(tx))
