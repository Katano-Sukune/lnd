...
func FallbackAddr(fallbackAddr xpcutil.Address) func(*Invoice) {
	return func(i *Invoice) {
...
// on-chain address.
func parseFallbackAddr(data []byte, net *chaincfg.Params) (xpcutil.Address, error) {
	...
	case 0:
		...
		switch len(witness) {
		case 20:
			addr, err = xpcutil.NewAddressWitnessPubKeyHash(witness, net)
		case 32:
			addr, err = xpcutil.NewAddressWitnessScriptHash(witness, net)
		default:
	...
	case 17:
		...
		addr, err = xpcutil.NewAddressPubKeyHash(pubKeyHash, net)
		if err != nil {
			return nil, err
		}
	case 18:
		...
		addr, err = xpcutil.NewAddressScriptHashFromHash(scriptHash, net)
		if err != nil {
			return nil, err
		}
	default:
...
func writeTaggedFields(bufferBase32 *bytes.Buffer, invoice *Invoice) error {
	...
	if invoice.FallbackAddr != nil {
		...
		case *xpcutil.AddressPubKeyHash:
			version = 17
		case *xpcutil.AddressScriptHash:
			version = 18
		case *xpcutil.AddressWitnessPubKeyHash:
			version = addr.WitnessVersion()
		case *xpcutil.AddressWitnessScriptHash:
			version = addr.WitnessVersion()
		default:
