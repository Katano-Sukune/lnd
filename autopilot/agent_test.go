...
func (m *mockConstraints) ChannelBudget(chans []Channel,
	balance xpcutil.Amount) (xpcutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() xpcutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() xpcutil.Amount {
	return 1e8
}
...
func (m *mockHeuristic) NodeScores(g ChannelGraph, chans []Channel,
	chanSize xpcutil.Amount, nodes map[NodeID]struct{}) (
...
func (m *mockChanController) SpliceIn(chanPoint *wire.OutPoint,
	amt xpcutil.Amount) (*Channel, error) {
	return nil, nil
}
...
func setup(t *testing.T, initialChans []Channel) (*testContext, func()) {
	...
	var availableFunds xpcutil.Amount = 10 * xpcutil.SatoshiPerBitcoin
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := Channel{
		ChanID:   randChanID(),
		Capacity: xpcutil.SatoshiPerBitcoin,
	}
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * xpcutil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []Channel{
		...
		{
			ChanID:   randChanID(),
			Capacity: xpcutil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += xpcutil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * xpcutil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != xpcutil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					xpcutil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * xpcutil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, xpcutil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget xpcutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation xpcutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation xpcutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget xpcutil.Amount = 5 * xpcutil.SatoshiPerBitcoin
	...
	expectedAllocation := testCtx.constraints.MaxChanSize() * xpcutil.Amount(numNewChannels)
	...
	channelBudget = xpcutil.SatoshiPerBitcoin * 3 / 2
